...
public class LogicUtils {
    ...
    public static TradeHistory doLogic(Sheet sheet, GustosBotLogicParams args, ZonedDateTime t1, ZonedDateTime t2) {
        ...
        List<XBar> bars = sheet.moments.stream().filter(m -> m.bar.getBeginTime().isAfter(after) && m.bar.getBeginTime().isBefore(before)).map(m -> m.bar)
                .collect(Collectors.toList());
        return new ProfitCalculator().tradeHistory(logic, args, sheet.instrument(), barInterval, sheet.exchange().getFee(), bars, aa, false);
    }
    ...
    public static double shake(Sheet sheet, GustosBotLogicParams args, double k, ZonedDateTime t1, ZonedDateTime t2) {
        ...
        int cc = 0;
        for (int j = -1;j<=1;j+=2){
            for (Method f : fld) if (f.getReturnType()==Integer.class && f.getName().startsWith("get")){
                cc++;
                GustosBotLogicParams p = args.copyIt();
                try {
                    int ii = (Integer)f.invoke(p);
                    ii = (int)(1+k*j)*ii;
                    args.getClass().getDeclaredMethod("set"+f.getName().substring(3),Integer.class).invoke(p,ii);
