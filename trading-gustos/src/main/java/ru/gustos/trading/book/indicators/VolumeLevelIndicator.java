...
public class VolumeLevelIndicator extends Indicator{

    public VolumeLevelIndicator(IndicatorInitData data){
        super(data);
    }
...
//                if (powOpen<min) continue;
                ...
                if (targetmax>=powMin && targetmax<=powMax)
                    money = true;
                else if (stoploss>=powMin && stoploss<=powMax)
                    money = true;
//                else if (visitMiddle && buypow>=powMin && buypow<=powMax)
//                    money = true;
                if (!money) {
